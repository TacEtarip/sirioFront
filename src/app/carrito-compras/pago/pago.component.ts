...
export class PagoComponent implements OnInit {
  ...
  setForm(user: FullUser): void {
    ...
    this.formContacto = this.fb.group({
      nombre: this.fb.control({ value: user.nombre, disabled: false }, Validators.compose([
        Validators.required,
        Validators.pattern(/^[a-zA-Z\s]+$/),
        Validators.minLength(2),
        Validators.maxLength(35)
      ])),
      apellido: this.fb.control({ value: user.apellido, disabled: false }, Validators.compose([
        Validators.required,
        Validators.pattern(/^[a-zA-Z\s]+$/),
        Validators.minLength(2),
        Validators.maxLength(35)
      ])),
      documento: this.fb.control({ value: user.documento || '', disabled: false }, Validators.compose([
        Validators.required,
        Validators.pattern(/^[0-9]{8}$|^[0-9]{11}$/)
      ])),
      documentoShow: this.fb.control({ value: '', disabled: true }),
      email: this.fb.control({ value: user.email || '', disabled: false }, Validators.compose([
        Validators.required,
        Validators.email,
        Validators.maxLength(50)
      ])),
      celular: this.fb.control({ value: user.celular || '', disabled: false }, Validators.compose([
        Validators.required,
        Validators.pattern
    ...
    this.formContacto.get('documento').valueChanges.pipe(distinctUntilChanged(), debounceTime(500)).subscribe(doc => {
      if (doc.length === 8) {
        ...
        this.inv.getRUC(doc).subscribe(ruc => {
          ...
          this.documentCharged.next(false);
          if (ruc) {
            this.formContacto.get('documentoShow').setValue(ruc.nombre_o_razon_social);
            this.formContacto.get('documentoShow').disable();
          } else {
            this.formContacto.get('documentoShow').setValue('No Encontrado');
            this.formContacto.get('documento').setErrors({incorrect: true});
