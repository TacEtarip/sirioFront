...
export class CotizacionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.inventarioMNG
      .getCotis(10, 0, 'noone', 'noone', this.estadoCoti$.value)
      .subscribe((res) => {
    ...
    this.inventarioMNG
      .getCotis(0, 0, 'noone', 'noone', this.estadoCoti$.value)
      .subscribe((res) => {
        ...
        (res: {
          ...
          if (res) {
            ...
            secondRef
              .afterClosed()
              .pipe(first())
              .subscribe((resTwo: { message: string; coti: Venta }) => {
                if (resTwo.coti) {
                  this.router.navigateByUrl(
                    `/ventas/cotizaciones/${resTwo.coti.codigo}`
                  );
                } else {
