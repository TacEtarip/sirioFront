...
export class ChangeOrderComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<ChangeOrderComponent>, private inv: InventarioManagerService,
              @Inject(MAT_DIALOG_DATA)
              public data: { categorias: Tipo[], items: Item[],
  ...
  activateScroll(event) {
    if (this.dragging) {
      this.timerSubject = timer(0, 50).subscribe(x => {
        const leftToScroll = this.divToScroll.nativeElement.scrollHeight - this.divToScroll.nativeElement.offsetHeight;
        if (leftToScroll < 0) {
  ...
  dropItems(event: CdkDragDrop<Tipo[]>) {
    this.changed.next(false);
    moveItemInArray(this.mockListItems, event.previousIndex, event.currentIndex);
  }
  ...
  cambiarOrden() {
    ...
    if (this.data.categorias) {
      ...
      this.inv.reOrderSubTipos(this.data.subCategorias.tipoCodigo, this.linksList, this.mockListST).subscribe(res => {
        this.changing.next(false);
        if (res) {
          this.changed.next(true);
