...
export class StoreMainComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    const isSmallScreenObs = breakpointObserver.observe([
      '(max-width: 650px)',
      '(max-width: 1100px)',
      '(max-width: 1300px)',
    ]);
  ...
  ngOnInit(): void {
    ...
    this.busquedaSub = this.busqueda
      .get('name')
      .valueChanges.subscribe((changeV: any) => {
        this.item$.next(null);
        if (changeV) {
  ...
  goToProducto() {
    this.router.navigate([
      'store',
      'categorias',
      this.item$.value.tipo,
      this.item$.value.subTipo,
      this.item$.value.codigo,
    ]);
  }

  filterItemValue(value: any) {
    this.inv
      .getListOfItemsFilteredByRegex(value.name || value, 15)
      .subscribe((res) => {
        if (res) {
          ...
          if (res.length === 0) {
            ...
            this.filteredItem$.value.findIndex((el) =>
              el.name.toUpperCase() === testValue
  ...
  resetAnimations() {
    ...
    this.toShowNext$.next(
      this.itemsDestacados$.value.length - this.cantidadSlider
    );
