...
import { InventarioManagerService, Tipo } from 'src/app/inventario-manager.service';
  ...
  openCambiarImagen() {
    ...
    dialogRef.componentInstance.onUploaded.pipe(first()).subscribe(res => {
      if (res) {
        this.tipo = res;
        this.tipoName$.next(res.name);
        this.urlOfImage.next('https://siriouploads.s3.amazonaws.com/' + `${res.link.split('.')[0]}.webp`);
      }
    });
  }
}
