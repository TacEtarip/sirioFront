...
@Component({
  ...
  styleUrls: ['./politica-de-envio.component.css']
})
export class PoliticaDeEnvioComponent implements OnInit {

  descripcion = 'Política de envio. Tienda de indumentaria de seguridad a el mejor precio en Trujillo, venta al por menor y al por mayor;' +
  ...
  ngOnInit(): void {
    this.titleService.setTitle('Política de envio | Sirio Dinar');
    this.metaTagService.updateTag(
      { name: 'description', content: this.descripcion }
    );
    this.metaTagService.updateTag
    ({ property: 'og:url', content: 'https://inventario.siriodinar.com/servicio-al-cliente/politica-de-envio' });
    this.metaTagService.updateTag({ property: 'og:title', content: 'Política de envio | Sirio Dinar' });
